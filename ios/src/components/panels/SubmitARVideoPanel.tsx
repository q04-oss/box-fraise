...
function AbstractForm({ onSubmitted }: { onSubmitted: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await submitARVideoAbstract(abstract.trim(), selectedTag);
      onSubmitted();
    } catch (err: any) {
  ...
  return (
    <>
      ...
      <Text style={[styles.hint, { color: c.muted, fontFamily: fonts.dmSans }]}>
        Describe your AR video concept. We commission videos case by case and pay on publication.
      </Text>
...
function UploadForm({ video, onSubmitted }: { video: any; onSubmitted: () => void }) {
  const c = useColors();
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [submitting, setSubmitting] = useState(false);

  const canSubmit = title.trim().length > 0 && description.trim().length >= 20 && !submitting;

  const handleSubmit = async () => {
    if (!canSubmit) return;
    setSubmitting(true);
    try {
      await uploadARVideo(video.id, title.trim(), description.trim(), '');
      onSubmitted();
    } catch {
      Alert.alert('Error', 'Something went wrong. Please try again.');
    } finally {
    ...
    <>
      {video.editor_note ? (
        <View style={[styles.editorNote, { borderColor: c.accent, backgroundColor: `${c.accent}10` }]}>
          ...
          <Text style={[styles.editorNoteText, { color: c.text, fontFamily: fonts.dmSans }]}>{video.editor_note}</Text>
        </View>
      ) : null}
      ...
      {video.abstract ? (
        <Text style={[styles.abstractPreview, { color: c.muted, fontFamily: fonts.dmSans, borderColor: c.border }]} numberOfLines={4}>
          Abstract approved: {video.abstract}
        </Text>
      ) : null}
      ...
      <View style={[styles.infoBox, { borderColor: c.border }]}>
        <Text style={[{ color: c.muted, fontFamily: fonts.dmSans, fontSize: 13, lineHeight: 19 }]}>
          Video file upload is handled via the creator dashboard at fraise.market
        </Text>
      </View>
...
export default function SubmitARVideoPanel() {
  ...
  // Derive state from videos
  const pendingAbstract = myVideos.find(v => v.status === 'abstract_submitted');
  const commissioned = myVideos.find(v => v.status === 'commissioned');
  const pastVideos = myVideos.filter(v =>
    !['abstract_submitted', 'commissioned'].includes(v.status)
  );
  ...
  const renderBody = () => {
    ...
    if (pendingAbstract) {
      return (
        <View style={styles.waitingContainer}>
          <Text style={[styles.waitingKanji, { color: c.accent }]}>映</Text>
          ...
          <Text style={[styles.waitingBody, { color: c.muted, fontFamily: fonts.dmSans }]}>
            We've reviewed your concept. We'll be in touch.
          </Text>
    ...
    // Commissioned — show upload form
    if (commissioned) {
      return <UploadForm video={commissioned} onSubmitted={load} />;
    }
        ...
        {!loading && pastVideos.length > 0 && (
          <>
            <View style={[styles.divider, { borderTopColor: c.border }]}>
              <Text style={[styles.sectionLabel, { color: c.muted, fontFamily: fonts.dmMono }]}>MY VIDEOS</Text>
            </View>
            {pastVideos.map(item => {
...
const styles = StyleSheet.create({
  ...
  waitingTitle: { fontSize: 22, marginBottom: SPACING.sm },
  ...
  abstractPreview: {
    ...
    borderRadius: 8, padding: SPACING.sm, opacity: 0.6, marginBottom: SPACING.md,
  },
  // Editor note
  editorNote: {
  ...
  bodyInput: {
    fontSize: 15, lineHeight: 22, height: 120,
    borderWidth: StyleSheet.hairlineWidth, borderRadius: 10,
