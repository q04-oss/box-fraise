...
export async function memberAppleSignIn(identityToken: string, name: string, email: string): Promise<FraiseMember & { token: string }> {
  const r = await fetch(`${FRAISE}/members/apple-signin`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ identityToken, name, email }),
  });
  const data = await r.json();
  if (!r.ok) throw new Error(data.error || 'apple sign in failed');
  return data;
}
