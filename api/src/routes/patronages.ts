...
router.post('/:id/claim', requireUser, async (req: any, res: Response) => {
  ...
  try {
    ...
    const [held] = await db
      ...
      .where(and(eq(seasonPatronages.id, id), eq(seasonPatronages.status, 'available')))
    ...
    try {
      ...
      await db.update(seasonPatronages).set({ status: 'available', patron_user_id: null }).where(eq(seasonPatronages.id, id));
      throw piErr;
    }
