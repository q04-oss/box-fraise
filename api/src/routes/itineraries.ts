...
// PATCH /api/itineraries/proposals/:id/accept
router.patch('/proposals/:id/accept', requireUser, async (req: Request, res: Response) => {
  const userId = (req as any).userId as number;
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({ error: 'invalid id' }); return; }
  try {
    const [proposal] = await db.select().from(itineraryProposals)
      .where(and(eq(itineraryProposals.id, id), eq(itineraryProposals.user_id, userId)));
    if (!proposal) { res.status(404).json({ error: 'not found' }); return; }
    if (proposal.status !== 'pending') { res.status(409).json({ error: 'already responded' }); return; }

    // Mark accepted + credit value to ad_balance
    await db.update(itineraryProposals)
      .set({ status: 'accepted', responded_at: new Date() })
      .where(eq(itineraryProposals.id, id));

    await db.update(users)
      .set({ ad_balance_cents: sql`${users.ad_balance_cents} + ${proposal.value_cents}` })
      .where(eq(users.id, userId));

    // Notify shop
    const [sender] = await db.select({
      push_token: users.push_token,
      business_name: businesses.name,
    }).from(users)
      .leftJoin(businesses, eq(users.business_id, businesses.id))
      .where(eq(users.business_id, proposal.business_id))
      .limit(1);
    ...
    res.json({ ok: true, credited_cents: proposal.value_cents });
  } catch (err) {
...
router.patch('/proposals/:id/decline', requireUser, async (req: Request, res: Response) => {
  ...
  try {
    const [updated] = await db.update(itineraryProposals)
      ...
      .where(and(eq(itineraryProposals.id, id), eq(itineraryProposals.user_id, userId), eq(itineraryProposals.status, 'pending')))
      .returning();
...
// GET /api/itineraries/region — shop users see itinerary demand in their area
// Returns count of active itineraries with destinations in this country/city
router.get('/region', requireUser, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Count unique users with destinations in this region
