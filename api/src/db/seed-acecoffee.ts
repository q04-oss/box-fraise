import { db } from './index';
import { businesses, locations } from './schema';
import { eq } from 'drizzle-orm';

async function run() {
  const existing = await db.select().from(businesses).where(eq(businesses.name, 'Ace Coffee'));
  if (existing.length > 0) { console.log('already exists, id:', existing[0].id); process.exit(0); }

  const [biz] = await db.insert(businesses).values({
    name: 'Ace Coffee',
    type: 'partner',
    address: '11053 86 Avenue NW',
    city: 'Edmonton, AB',
    hours: 'Daily 7–3',
    latitude: '53.5228',
    longitude: '-113.5061',
    launched_at: new Date('2026-01-01'),
    neighbourhood: 'McKernan',
    approved_by_admin: true,
  }).returning();

  await db.insert(locations).values({
    name: 'Ace Coffee',
    address: '11053 86 Avenue NW, Edmonton, AB T6G 0X1',
    active: true,
    business_id: biz.id,
  });

  console.log('Created:', biz.name, 'id:', biz.id);
  process.exit(0);
}

run().catch(e => { console.error(e); process.exit(1); });
