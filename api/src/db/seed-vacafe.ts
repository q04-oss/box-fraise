...
async function run() {
  const existing = await db.select().from(businesses).where(eq(businesses.name, 'Va Cafe'));
  if (existing.length > 0) { console.log('already exists, id:', existing[0].id); process.exit(0); }

  const [biz] = await db.insert(businesses).values({
    name: 'Va Cafe',
    type: 'partner',
    address: '12024 Jasper Avenue',
    city: 'Edmonton, AB',
    hours: 'Daily 8–3',
    contact: '780-249-2445',
    latitude: '53.5449',
    longitude: '-113.5214',
  ...
  await db.insert(locations).values({
    name: 'Va Cafe',
    address: '12024 Jasper Avenue, Edmonton, AB',
    active: true,
    business_id: biz.id,
  });
