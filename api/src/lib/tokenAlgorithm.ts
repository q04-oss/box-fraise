...
export function computeTokenVisuals(seed: number, excessCents: number): TokenVisuals {
  ...
  // Use Math.max(1.01, dollars) so CA$1 excess (dollars=1) yields log10(1.01)>0 and ease>0
  const dollars = excessCents / 100;
  const logMax  = 7; // log10(10,000,000) — beyond this, max
  const logVal  = Math.log10(Math.max(1.01, dollars));
...
export async function getNextTokenNumber(
  ...
  // Use MAX(token_number)+1 to avoid race conditions under concurrent mints
  // and to remain correct after deletions (COUNT would reuse numbers)
